...
public class PKCS7Util {
	...
	private static Logger log = LoggerFactory.getLogger(PKCS7Util.class);
	 ...
	 * 私有构造方法
	 */
	private PKCS7Util(int mode) {
		this.mode = mode;
	}
	
	public static void setDebug(boolean debug) {
		PKCS7Util.debug = debug;
	}
	...
	private static void init() throws ClassNotFoundException {
		...
		try {
			if (vendorUC.indexOf("SUN") >= 0) {
			...
			else if(vendorUC.indexOf("ORACLE") >= 0){
				jvm = 'O';
				algorithmId = Class.forName("sun.security.x509.AlgorithmId");
				derValue = Class.forName("sun.security.util.DerValue");
				objectIdentifier = Class.forName("sun.security.util.ObjectIdentifier");
				...
				throw new ClassNotFoundException("Not support JRE: " + vendor);
			}
	...
	public static PKCS7Util getSignerInstance() {
		PKCS7Util instance;
		try {
			instance = getSigner(Property.getProperty(Constants.BOC_B2B_PFXCER),
					Property.getProperty(Constants.BOC_B2B_PFXPASS),
					Property.getProperty(Constants.BOC_B2B_PFXPASS));
	 ...
	 */
	public static PKCS7Util getSigner(String keyStorePath, String keyStorePassword, String keyPassword)
		...
		PKCS7Util tool = new PKCS7Util(SIGNER);
		tool.certificates = certificates;
		tool.privateKey = privateKey;
		return tool;
	}
	
	public static PKCS7Util getVerifierInstance() {
		PKCS7Util instance;
		try {
			instance = getVerifier(Property.getProperty(Constants.BOC_B2B_SZBOCCER));
		} catch (ClassNotFoundException | GeneralSecurityException
				| IOException e) {
			instance = null;
			log.error(e.getMessage(),e);
	 ...
	 * @throws ClassNotFoundException 
	 */
	public static PKCS7Util getVerifier(String rootCertificatePath)
			throws GeneralSecurityException, IOException, ClassNotFoundException {
		init();
		//加载根证书
		FileInputStream fis = null;
		...
		PKCS7Util tool = new PKCS7Util(VERIFIER);
...
//		  public PKCS7(sun.security.x509.AlgorithmId[] arg0, sun.security.pkcs.ContentInfo arg1, java.security.cert.X509Certificate[] arg2, sun.security.pkcs.SignerInfo[] arg3);
	...
	public boolean verify(String signature, String data) throws Exception {
		return verify(signature, data, StringUtils.EMPTY);
	}

	public boolean verify(String signature, String data, String dn) throws Exception {
		return verify(signature, data.getBytes("UTF-8"), dn);
	}
	 ...
	 */
	public boolean verify(String signature, byte[] data, String dn) throws IOException, NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException, NoSuchProviderException  {
		if (mode != VERIFIER)
		...
		for (int i = 0; i < sis.length; i ++) {
			...
			//验证dn
			if (i == 0 && StringUtils.isNotEmpty(dn)) {
